...
The first search for damage looked in the wrong place. Comparisons, `during`,
`overlaps`, `duration in` and `difference in` all answer correctly across the
boundary, because the operators convert as they go — which is why this sat here
described as harmless. The damage is where the type *is* the question:
